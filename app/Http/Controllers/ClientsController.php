...
class ClientsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Run aggregation
        $Clients = Clients::raw(function ($collection) use ($matchCondition, $skip, $limit) {
            $pipeline = array_merge($matchCondition, [
                // Lookup country
                ['$lookup' => [
                    'from' => 'countries',
                ...
                // Lookup related projects using client_id

                ['$lookup' => [
                    'from' => 'projects',
                    'let' => ['statusId' => ['$toString' => '$_id']],
                    'pipeline' => [
                        ['$match' => ['$expr' => ['$eq' => ['$client_id', '$$statusId']]]],
                        [
                            '$lookup' => [
                ...
                ['$addFields' => [
                    'total_projects' => [
                        '$size' => ['$ifNull' => ['$project_data', []]]  // Default to an empty array if project_data is missing
                    ],
                    'in_progress_projects' => [
                        '$size' => [
                            '$filter' => [
                                'input' => ['$ifNull' => ['$project_data', []]],  // Ensure it's an array
                                'as' => 'proj',
                                'cond' => [
                                    '$eq' => [
                                        '$$proj.project_status.name',
                                        'In Progress'
                                    ]
                                ]
                            ]
        ...
        $totalCount = Clients::raw(function ($collection) use ($matchStage) {
            $matchFilter = [];
            if (!empty($matchStage)) {
    ...
    // Get project summary by client id
    public function project_summary($id, Request $request)
    {
        $aggregation = Project::raw(function ($collection) use ($id) {
            return $collection->aggregate([
                ['$match' => ['client_id' => $id]],
                ['$addFields' => [
                    'status_obj_id' => ['$toObjectId' => '$project_status_id']
                ]],
                ['$lookup' => [
                    'from' => 'project_statuses',
                    'localField' => 'status_obj_id',
                    'foreignField' => '_id',
                    'as' => 'status'
                ]],
                ['$unwind' => [
