...
class UserController extends Controller
{
    public function addUser(Request $request)
    {
        ...
        try {
            ...
            Mail::to($user->email)->send(new WelcomeOnboardMailEmployee($user));
    ...
    public function getAllUsers(Request $request)
    {
        ...
        $totalCount = User::raw(function ($collection) use ($filters) {
            return $collection->aggregate([
                ['$match' => count($filters) > 0 ? $filters : (object)[]],
                ['$count' => 'total']
            ]);
        })->first()['total'] ?? 0;
    ...
    public function getAllUsersNotAdmins()
    {
        ...
        $query = User::select('_id', 'name', 'last_name', 'email', 'contact_number', 'profile_photo', 'employee_id');

        if ($adminRole) {
            $query->where('role_id', '!=', (string)$adminRole->_id);
        }
    ...
    public function updateUserPassword(Request $request)
    {
        ...
        // User existence check is now implicitly handled by the 'exists:users,_id' validation rule
        // No need for an explicit if (!$user) check here as validation already covers it.
